...
class IspaManager:
    ...
    def download_and_apply_fix(self, appid: int, fix_type: str) -> Dict[str, Any]:
        """Önce Global-OnlineFix-Unsteam-main klasöründen fix ara, bulunamazsa generator.ryuu.lol'den indir"""
        try:
            appid = int(appid)
            fix_type = str(fix_type).lower()
        except (ValueError, TypeError):
        ...
        try:
            ...
            # Önce Global-OnlineFix-Unsteam-main klasöründen fix ara
            plugin_dir = os.path.dirname(os.path.dirname(stplug_path))  # backend klasörünün üstü
            ...
            if os.path.exists(global_fix_dir):
                logger.log(f'ispa: Checking Global-OnlineFix-Unsteam-main for {fix_type} fix')

                # Fix tipine göre uygun klasörü bul
                fix_mappings = {
                ...
                target_dirs = fix_mappings.get(fix_type, [])
                        ...
                        try:
                            for item in os.listdir(source_path):
                                ...
                                if os.path.isdir(s):
                                    ...
                                    shutil.copy2(s, d)
                            logger.log(f'ispa: {fix_type} fix copied from Global-OnlineFix-Unsteam-main to {extract_path}')
                            fix_applied = True
                            break
                        except Exception as copy_error:
                            logger.warn(f'ispa: Failed to copy from Global-OnlineFix-Unsteam-main: {copy_error}')
                            continue

            # Global-OnlineFix-Unsteam-main'den fix bulunamadıysa generator.ryuu.lol'den indir
            if not fix_applied:
                logger.log(f'ispa: {fix_type} fix not found in Global-OnlineFix-Unsteam-main, trying generator.ryuu.lol')
            ...
            if fix_applied and extract_path:
                ...
                logger.log(f'ispa: {fix_type} fix applied to appid {appid}')
                return {'success': True, 'message': f'{fix_type} fix başarıyla uygulandı', 'path': extract_path}
            else:
                return {'success': False, 'error': f'{fix_type} fix bulunamadı veya uygulanamadı'}
